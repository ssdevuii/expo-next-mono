...
const EditTeamForm = () => {
  ...
  useEffect(()=> {
    ...
    if(project.isSuccess){
        ...
        const docState = convertFromRaw(JSON.parse(project.data?.description ?? ""))

        setKaryaName(project.data?.name ?? "")
        setDescription(EditorState.createWithContent(docState))
        setCategory(project.data?.categoryId ?? 1)
        setVideoURL(project.data?.videoLink ?? "")
        setDemoURL(project.data?.demoLink ?? "")
        setImgUrl(project.data?.gdriveLink ?? "")
        
        
    }
  },[project.data?.categoryId, project.data?.demoLink, project.data?.description, project.data?.gdriveLink, project.data?.name, project.data?.videoLink, project.isSuccess])
